...
export default function RegistrarUnidade(props) {
  ...
  function verificaInformacoes({ name, email, descricao }) {
    ...
    .then(async (response) => {
      ...
      await console.log(resposta.status);
      if(resposta.status === 'ok'){
       return alert('Unidade cadastrada com sucesso');
      }else if (await resposta.status === 'erro'){
          ...
          <Grid container>
            <Grid item xs={12} className={classes.dropdown}>
              <FormControl variant="filled" className={classes.buttonSelect}>
                <InputLabel htmlFor="filled-dia-native-simple">
                  Tipo de serviço
                </InputLabel>
                <Select
                  native
                ...
                >
                  ...
                  <option>Vacina Covid-19 - 2° dose (ASTRAZENECA)</option>
                  <option>Vacinação/Influenza</option>
                  <option>Outras vacinas - Crianças e Adultos</option>
                </Select>
              </FormControl>
            </Grid>
          </Grid>
